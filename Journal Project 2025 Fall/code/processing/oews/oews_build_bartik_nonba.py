...
def find_source():
    ...
    raise FileNotFoundError(f"No OEWS sources in {RAW}")
...
def build_from_all_data(path: pathlib.Path) -> pl.DataFrame:
    ...
    cb_pre = (df.filter(pl.col("areatype_code")=="M")
                .with_columns(pl.col("area_code").cast(pl.Utf8).str.zfill(5).alias("cbsa"))
                .select(["cbsa","occ_code","year","value"])
                .with_columns(pl.col("occ_code").str.slice(0,2).is_in(list(NONBA_MAJOR)).cast(pl.Int8).alias("non_ba")))
    base_year = choose_base_year(cb_pre)

    base = cb_pre.filter((pl.col("non_ba")==1) & (pl.col("year")==base_year))
...
def build_from_excel(zips: list[pathlib.Path]) -> pl.DataFrame:
    ...
    for zp in zips:
        ...
        if not (area_code and occ_code and emp_col):
            raise RuntimeError(f"Missing needed columns in {zp.name}. Found: {list(df.columns)[:10]} ...")
    ...
    big = (big.filter(pl.col("occ_code").is_not_null() & pl.col("value").is_not_null())
              .filter(pl.col("occ_code") != "00-0000")
              .with_columns(
                  pl.col("area_code").cast(pl.Utf8).alias("area_code"),
                  pl.when(pl.col("areatype_code").is_not_null())
                    .then(pl.col("areatype_code").cast(pl.Utf8))
                    .otherwise(
                        pl.when(pl.col("area_code").str.len_bytes()==5).then(pl.lit("M"))
                         .when(pl.col("area_code").str.len_bytes()==2).then(pl.lit("S"))
                         .otherwise(pl.lit(None))
                    ).cast(pl.Utf8).alias("areatype_code"),
                  pl.col("occ_code").cast(pl.Utf8).alias("occ_code"),
                  pl.col("year").cast(pl.Int32).alias("year"),
              ))


    us = (big.filter(pl.col("areatype_code")=="S")
    ...
    cb = (big.filter(pl.col("areatype_code")=="M")
             .with_columns(pl.col("area_code").str.zfill(5).alias("cbsa"))
             .select(["cbsa","occ_code","year","value"])
             .with_columns(pl.col("occ_code").str.slice(0,2).is_in(list(NONBA_MAJOR)).cast(pl.Int8).alias("non_ba")))
...
def main():
    ...
    print(f"[INFO] Source mode: {kind} | {src if isinstance(src, pathlib.Path) else [p.name for p in src]}")
    bartik_nb = build_from_all_data(src) if kind=="all_data" else build_from_excel(src)
    if bartik_nb.height == 0: raise RuntimeError("Placebo Bartik result is empty.")

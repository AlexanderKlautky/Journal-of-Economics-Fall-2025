...
OUT_INT   = INT/"u_28_35_ba_cbsa_q.parquet"
...
def file_month_aggregate(zpath: Path) -> pl.DataFrame | None:
    ...
    if educ.lower() == "peeduca":
        ba_plus = (pl.col(educ).cast(pl.Int16, strict=False) >= 43)
    else:
        ba_plus = pl.col(educ).cast(pl.Utf8).str.contains("Bachelor|Master|Prof|Doctor", literal=True, strict=False)

    st = pl.col(stat).cast(pl.Int16, strict=False)
    in_lf = st.is_in([1,2,3,4]) & ba_plus & pl.col("age_i").is_between(28,35)
    unemp = st.is_in([3,4])     & ba_plus & pl.col("age_i").is_between(28,35)
...
def build_all() -> pl.DataFrame:
    ...
    q = (m.group_by(["cbsa5","year","quarter"])
           ...
           .with_columns(u_28_35_ba = (pl.col("u_w")/pl.col("lf_w")).clip(0.0,1.0))
           .select([pl.col("cbsa5").alias("cbsa"), "year","quarter","u_28_35_ba"])
...
def save_outputs(uq: pl.DataFrame):
    ...
    if OUT_FINAL.exists():
        ...
        conflict_names = {
            "cbsa_r", "year_r", "quarter_r",
            "cbsa_right", "year_right", "quarter_right",

            "u_28_35_ba_r", "u_28_35_ba_right"
        }
        renames = {c: f"{c}__prev" for c in add.columns if c in conflict_names}
        if renames:
            add = add.rename(renames)


        if "cbsa" in add.columns and "cbsa" in uq.columns:
            try:
                uq = uq.with_columns(pl.col("cbsa").cast(add.schema["cbsa"], strict=False))
            except Exception:
                add = add.with_columns(pl.col("cbsa").cast(pl.Utf8, strict=False))
                uq  = uq.with_columns(pl.col("cbsa").cast(pl.Utf8, strict=False))
        if "year" in add.columns and "year" in uq.columns:
            try:
                uq = uq.with_columns(pl.col("year").cast(add.schema["year"], strict=False))
            except Exception:
                pass
        if "quarter" in add.columns and "quarter" in uq.columns:
            try:
                uq = uq.with_columns(pl.col("quarter").cast(add.schema["quarter"], strict=False))
            except Exception:
                pass


        uq = uq.select(["cbsa","year","quarter","u_28_35_ba"])


        out = (add.join(uq, on=["cbsa","year","quarter"], how="full", suffix="_r2")
...
def main():
    uq = build_all()
    save_outputs(uq)
    print(f" u_28_35_ba built: {uq.height:,} rows")
    print(f"   → {OUT_INT}")
